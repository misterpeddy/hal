...
class Track:
    ...
    def show_chromagram(self):
        plt.figure(figsize=_FIG_SIZE)
        rosa.display.specshow(self._chromagram.numpy(
        ).T, sr=_VID_FRAME_RATE, y_axis='chroma', x_axis='frames')
        plt.tight_layout()
        plt.show()
...
class Audio:
    ...
    def __init__(self, filename, autogen_stems=False):
        ...
        self.tracks['source'] = Track(
            filename, source_name, autogenerated=False)
        if not hasattr(self, 'gen_dir'):
    ...
    def cut(self, start, stop):
        ...
        output_path = io_utils.add_suffix(
            self.source_file, f'{start}_to_{stop}', self.gen_dir)
        (
            ffmpeg
...
class YouTubeAudio(Audio):
    ...
    def __init__(self, link, autogen_stems=False):
        video = YouTube(link)
        stream = video.streams.filter(only_audio=True).first()
        self.gen_dir = os.path.join(
            io_utils.create_cache_dir('youtube_audio'),
            io_utils.name_from_path(stream.title)
        )
        self.gen_dir = io_utils.next_dir(self.gen_dir)
        filename = stream.download(
            output_path=self.gen_dir, filename=f'{stream.title}.mp3')
        super(YouTubeAudio, self).__init__(filename, autogen_stems)
